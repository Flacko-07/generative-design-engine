#!/usr/bin/env python3
"""
Phase 2 – Corrected Parametric Cylinder in Cross-flow (snappyHexMesh)
Generates cylinder_dataset.csv with (diameter, x_center, Re, Cd)

Fixes applied vs original:
  1. Forces file path corrected (postProcessing/forces/<time>/force.dat)
  2. STL side-facet normals are now proper outward normals (not (0,0,0))
  3. All imports moved to top level
  4. val_str renamed to openfoam_value (consistent with other scripts)
  5. top/bottom channel walls changed from noSlip → slip (far-field BC)
  6. Reynolds number computed and logged per run
  7. Warning raised when Re > 2000 (flow is turbulent, laminar solver invalid)
  8. Background mesh resolution derived from target_cell_size, not hardcoded
  9. Batch runner: try/except per case + incremental CSV save (no data loss on crash)
 10. forces function: log enabled, writeControl set explicitly
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# Batch runner  ── FIX: per-case try/except + incremental CSV save
# ──────────────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    diameters   = np.linspace(0.1, 0.5, 3)
    x_positions = np.linspace(1.0, 3.0, 3)

    # Laminar regime: U=0.001, nu=1e-5 → Re = 10 (D=0.1) to 50 (D=0.5)
    # Steady laminar flow gives physically meaningful Cd(D, x) data.
    U_inlet          = 0.001
    nu               = 1e-5
    target_cell_size = 0.05   # finer mesh improves accuracy at low Re

    output_csv = Path("cylinder_dataset.csv")
    ...
    for dia in diameters:
        for xc in x_positions:
            print(f"\n▶  D={dia:.3f}  x={xc:.3f} ...", end="  ", flush=True)
            try:
                ...
                # Append immediately so a later crash doesn't lose this result
    ...
    if failed:
        print(f"\n⚠️  {len(failed)} case(s) failed:")
        for dia, xc, err in failed:
            print(f"    D={dia:.3f}  x={xc:.3f}  →  {err}")